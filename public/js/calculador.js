...
function gather() {
  ...
  if (result === 0) {
    $(".results-data").append(
      '<p class="text-danger convert-emoji"> You didn\'t enter anything. Try again ya filthy animal.</p>'
    );
    $(".emoji").append('<i class="frown">&nbsp;</i>');
  } else if (result < 0) {
    $(".results-data").append(
      '<p class="text-danger"> After your expenses you have $' +
        result +
        " left in your budget. You might want to try and reduce your spending this month.</p>"
    );
    $(".emoji").append('<i class="tear">&nbsp;</i>');
  } else {
    $(".results-data").append(
      '<p class="text-sucess"> After your expenses you have $' +
        result +
        " left in your budget.</p>",
      '<p class="text-sucess">But you should save at least $' +
        savings +
        ".</p>"
    );
